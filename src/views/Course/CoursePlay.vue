...
import {playerService,recordHistory,getLastHistoryByChapterId} from '@/api/course'
...
const player=async(cid,Cid)=>{
  const res =await playerService({
    courseId:cid,
    chapterId:Cid
  })
  // 当前播放课程的数据
  ...
  bizCourseDetail.value=res.data.courseInfo.bizCourseDetail
  bizCourseTeacher.value=res.data.courseInfo.bizCourseTeacher
  activeId.value=chapterInfo.value.id
//  options.src=res.data.playInfo.playInfoList[0].playUrl
}
player(courseId,chapterId)

let time=ref(0)
// 更新播放时间事件----每个多长时间记录视频播放
const onTimeupdate=async(ev)=>{
  time.value++
  if(time.value>=10){
  const res=await recordHistory({
    chapterId,
    courseId,
...
const onLoadStart=async(ev)=>{
  const res =await getLastHistoryByChapterId({
    memberId:userStore.userInfo.id,
    courseId,
    chapterId
  })
...
<template>
	<div class="main">
		...
		<div class='play'>
			<div class='play-left'>
        <vue3VideoPlay @loadstart="onLoadStart" @timeupdate="onTimeupdate" v-bind="options"></vue3VideoPlay>
			...
			<div class='play-right'>
              ...
              <template #label>
	            ...
	            <div class="course-container" >
	              ...
	              <div class="courseDesc">{{bizCourseDetail.description}}</div>
	              <div class="courseImg">
	                <img :src="bizCourseDetail.courseCover" alt="" />
	              </div>
              ...
              <template #label>
	            ...
	            <div class="chapter-container">
	              <dl class="list" v-for="item in bizCourseChapters" :key="item.id">
	                ...
	                <dd
	                  :class="activeId===k.id?'active':''"
                    v-for="k in item.children" :key="k.id"
                    @click="toggleNodule(k.id)"
	                >
