...
import {settlement,deleteShopCars,alipayOrder,queryAlipay,wxpayOrder,queryWxpay} from '@/api/order'
...
const getOrder=async()=>{
  const res = await settlement(cartStore.order())
...
// 选择支付方式
const tabPayMent=async(item)=>{ 
  payment.value.description=item.description
  if(item.code==='alipayment'){
      const res= await alipayOrder({
      courses:cartStore.order(),
      payModes:item.code
    })
      payUrl.value=res.data.payurl
      orderNumber.value=res.data.orderNumber
  }else if(item.code==='wxpayment'){
     const res= await wxpayOrder({
      courses:cartStore.order(),
      payModes:item.code
    })
      payUrl.value=res.data.payurl
...
const queryZFB=()=>{
  ...
  if(res.meta.code==='200'){
    ...
    let arr=cartStore.selectArr()
    const data=await deleteShopCars(arr,token)
...
const goPayment=()=>{
  ...
  if(payment.value.description==='支付宝支付'){
    queryZFB()
  }else if(payment.value.description==='微信支付'){
    queryWX()
  }
}
