...
import {loginService,sendCaptchaService,loginByMobileService} from '@/api/login'
// 引入加密解密的方法
import {Decrypt,Encrypt} from "@/utils/aes"
import {useUserStore} from '@/stores/index'
import Verify from '@/views/verifition/Verify.vue'
...
const interCode=()=>{
    ...
    clearInterval(phomeTimer)
    phomeTimer=setInterval(()=>{
        timer--
        if(timer<=0){
            clearInterval(phomeTimer)
            captcha.value=`重新发送`
...
// 发送短信验证码
const sendCode=()=>{
   ruleFormRefPhone.value.validateField('phone',async(value)=>{
    if(!value){
...
const phoneBtn = (formEl) => {
	if (!formEl) return
 	formEl.validate( async (valid, fields) => {
	    if (valid) {
            let mobile=Encrypt(ruleFormPhone.phone)//加密手机号
            let captcha=ruleFormPhone.captcha
			const res =await loginByMobileService({
                mobile,
                captcha
            })
            if(res.meta.code==='10006'){
			...
			<div class="login-box">
	            ...
	            <div class="login-left">
	                <a href="/" title="小鹿线" class="logo">
	                    <img src="@/imgs/logo02.png" alt="小鹿线" title="小鹿线">
	                </a>
...
.container{
    ...
    width: 100vw;
    height: 100vh;
    background: url('@/imgs/loginbg.jpeg') no-repeat center;
}
section{
	position: absolute;
    top: 0;
...
.login-right{
    ...
    width: 475px;
    height: 500px;
    background: url("@/imgs/ybbg.jpeg") no-repeat center center;
}
